...
/*   Updated: 2025/07/19 06:04:45 by myli-pen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

static inline t_mat4	model_matrix(t_context *ctx);
...
bool	mvp(t_vertex *vert, t_context *ctx)
{
	...
	{
		m.m = mat4_mul(isometric_rot(), m.m);
		m.v = mat4_translate(vec3_scale(ctx->cam.target, -1.0f));
	}
...
static inline t_mat4	proj_ortho(t_cam cam)
{
	...
	dist = cam.distance;
	aspect = cam.aspect;
	sides = vec4(-dist * aspect, dist * aspect, -dist, dist);
